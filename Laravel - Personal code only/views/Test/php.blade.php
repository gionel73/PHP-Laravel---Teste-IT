<?php require_once('c:\xampp\htdocs\testit\resources\questions\phpquestions.php'); ?>

@extends('layouts.master')
@section('content')
	
	<b style="font-size:20px; margin-left:30px">Teste IT: &nbsp PHP</b>
	<br>
	...
	<legend style="border-top:5px maroon solid; border-right:5px maroon solid; border-radius:10px; padding-bottom:5px; padding-top:5px; width:650px">&nbspQuestion {{$nr+1}} : {{$questions[$nr][0]}}</legend></br>
		<a href="{{route('testit.addResultPhp',['val'=>'a','nr'=>$nr])}}"><button> a. </button></a> &nbsp{{$questions[$nr][1]}}<br><br>
		<a href="{{route('testit.addResultPhp',['val'=>'b','nr'=>$nr])}}"><button> b. </button></a> &nbsp{{$questions[$nr][2]}}<br><br>
		<a href="{{route('testit.addResultPhp',['val'=>'c','nr'=>$nr])}}"><button> c. </button></a> &nbsp{{$questions[$nr][3]}}<br><br>
	</fieldset>
